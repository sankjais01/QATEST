...
public class SmokeTest {
	...
	public void init() {
		...
		driver = BrowserFactory.createDriver("firefox");
		if (driver == null) {
			flag = false;
	...
	public void homePageTest() throws InterruptedException {
		...
		//System.out.println("Total arrivals present are - " + homepage.countArrivals());
		//System.out.println("Total slides present inside slider are - " + homepage.countSlider());
		...
		/*		try {
			...
			for (WebElement webElement : clickableLinks) {
				...
				if (webElement.getText().length() != 0) {
					System.out.println("navigating to link of text - " + webElement.getText());
					webElement.click();
					asert.assertFalse(driver.getCurrentUrl().equals(Locators.HOMEPAGE_URL), "Link not clickable");
			...
			System.out.println("total clickable links are  -  " + count);
		} catch (Exception e) {
