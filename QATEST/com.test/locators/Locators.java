package locators;

public class Locators {

	// pages URL

	public static final String HOMEPAGE_URL = "http://practice.automationtesting.in/";
	public static final String LOGINPAGE_URL = "";

	// home page object Xpath
	public static final String HOMELOGO_XPATH = ".//*[@id='site-logo']";
	public static final String MENUITEM_SHOP_XPATH = ".//*[@id='menu-item-40']/a";
	public static final String SLIDERFRAME_XPATH = ".//*[@id='n2-ss-6']";
	public static final String SLIDERARROW_PREVIOUS_XPATH = "//div[@id='n2-ss-6-arrow-previous']";
	public static final String SLIDERARROW_NEXT_XPATH = "//div[@id='n2-ss-6-arrow-next']";
	public static final String SLIDERONE_XPATH = "//div[contains(@class,'n2-ss-slide-36 n2-ss-slide-active')]";
	public static final String SLIDERTWO_XPATH = "//div[contains(@class,'n2-ss-slide-34 n2-ss-slide-active')]";
	public static final String SLIDERTHREE_XPATH = "//div[contains(@class,'n2-ss-slide-35 n2-ss-slide-active')]";

}
